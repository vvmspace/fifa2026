...
const matches = events.map(event => {
    ...
    const isKnockout = name.toLowerCase().includes('round of') || 
...
const indexGridContent = selectedGroupMatches.map((match, i) => generateMatchCard(match, i, 'index')).join('\n');
const boletosfifaGridContent = selectedGroupMatches.map((match, i) => generateMatchCard(match, i, 'boletosfifa')).join('\n');
const copa26GridContent = selectedGroupMatches.map((match, i) => generateMatchCard(match, i, 'copa26')).join('\n');
...
export { matches, selectedGroupMatches, selectedKnockoutMatches, generateMatchCard };
